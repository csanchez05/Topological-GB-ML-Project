...
# Compress to 2D
print("Scaling and performing PCA...")
...
# Pointing to the raw, unfiltered grid shifts
...
# Layer 1: The Universe (Uncalculated)
plt.scatter(df_universe['PC1'], df_universe['PC2'], 
            c='lightgray', alpha=0.3, s=20, label='Uncalculated Structures')

# Layer 2: The Old Data (Existing VASP Runs - 650 Structures)
plt.scatter(df_known['PC1'], df_known['PC2'], 
            c='midnightblue', alpha=0.6, s=50, linewidth=0.5,
            label=f'Existing Runs ({len(df_known)})')

# Layer 3: The New Targets (FPS Selections)
plt.scatter(df_fps['PC1'], df_fps['PC2'], 
            c='crimson', alpha=0.9, s=150, linewidth=1.5,
            label=f'New Targets ({len(df_fps)})')

plt.title("Phase Space Mapping: Farthest Point Sampling", fontsize=15)
...
output_png = "/home/calvi/Research_Group/ML_Interface_Project/results/FPS_Targets_Overlay_Unfiltered.png"
plt.tight_layout()
